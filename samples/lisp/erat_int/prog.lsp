...
(defun eratosfen (max)
   (do_eratosfen max nil (list_all_numbers max))
)

(defvar current_result nil)

(defun do_eratosfen (max ready remain)
   (setq current_result ready)
   (cond ((null remain) ready)
         (t (do_eratosfen 
               max
               (cons (car remain) ready) 
               (rm_list_from_list 
